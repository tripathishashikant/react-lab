...
- **Manual Git Control**: NEVER automate `git add`, `git commit`, or `gh pr create` without explicit user permission. Always present changes for **Manual Review** in Phase 8.
- **Code Hygiene**:
    - DO NOT add unnecessary or self-explanatory comments to the code.
    - ALWAYS use constants instead of hardcoded string values for state flags, settings, or magic strings (e.g., use `THEMES.DARK` instead of `'dark'`).
- **Feature Branching**: ALWAYS create a feature branch before starting any implementation. Branch names MUST follow the pattern: `<type>/<issue-number>-<description>`. Use the script `.gemini/hooks/validate-branch.sh` to verify.
...
_Focus on reliability, type safety, and maintainable architecture._
